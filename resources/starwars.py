...
def get_characters():
    ...
    #     character = Character_(**char)
    response = {"count": data.get("count"), "message": "successful"}
...
def post_films():
    ...
    if result:
        ...
        response_obj = {"message": "failed to insert record"}
        return Response(
...
def delete_films():
    ...
    if result == 0:
        response_obj = {"ERROR": f"no records to delete against film_id - {film_id}"}
        return Response(
            json.dumps(response_obj), status=409, mimetype="application/json"
        )
    response_obj = {
...
def put_films():
    ...
    absolute_url = home_url + relative_url.format(num_=film_data.film_id)
